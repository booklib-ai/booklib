...
function extractItems(text) {
  ...
  for (const block of blocks) {
    ...
    items.push({ label: null, body: block.slice(0, 250), raw: block });
  }
  ...
  return items
    ...
    .filter(item => {
      ...
      if (/^[-─═]{5,}/.test(b)) return false;               // horizontal rule
...
export class ContextBuilder {
  ...
  async _searchAndGroup(queries) {
    ...
    for (const query of queries) {
      let results = [];
      try { results = await this._searcher.search(query, 8, 0.35); } catch { /* not indexed */ }
      for (const chunk of results) {
        ...
        // Prefer principled sections (framework/core_principles) over examples
        const principled = ['framework', 'core_principles', 'pitfalls', 'anti_patterns', 'content'];
        const newType = chunk.metadata?.type ?? '';
        ...
        if (
          (newIsPrincipled && !existingIsPrincipled) ||        // always prefer principled
          (newIsPrincipled === existingIsPrincipled && scoreDelta > 0) // else take higher score
        ) {
          bySkill.set(skillName, { ...chunk, _skill: skillName });
        }
      }
  ...
  _compileReport(task, winners, suppressionMap, bySkill, decisions) {
    ...
    for (const w of winners) {
      ...
      lines.push(`  📖 ${book}`);
      lines.push(`     Section: ${section}   Relevance: ${score}`);
      lines.push('');
