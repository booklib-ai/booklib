...
async function stepProcessingMode(ui, cwd) {
  ...
  const options = [
    { value: 'fast', label: 'Fast (default)', hint: 'Instant results, no AI reasoning — works everywhere' },
  ];
  ...
  if (hasApiKey) {
    options.push({ value: 'api', label: 'Cloud AI', hint: `Uses your ${keyProvider} API key — best quality, ~$0.001/query` });
  } else {
    options.push({ value: 'api', label: 'Cloud AI', hint: 'Requires API key (Anthropic or OpenAI) — best quality' });
  }
