...
async function stepProcessingMode(ui) {
  ...
  if (mode === 'api' && !hasApiKey) {
    const provider = await ui.select('Which AI provider do you use?', [
    ...
    if (provider !== 'skip') {
      ...
      const key = await ui.text(`Paste your ${envVar}:`, 'sk-...');
      ...
      if (key && key.length > 10) {
        ...
        // Also set for current process so it works immediately
        process.env[envVar] = key;

        ui.log.success(`${envVar} saved to .env`);
        ui.log.info('Make sure .env is in your .gitignore!');
        ...
        if (fs.existsSync(gitignorePath)) {
          ...
          if (!gitignore.includes('.env')) {
            ...
            ui.log.success('Added .env to .gitignore');
          }
        }
      } else {
        ui.log.warn('No key provided. BookLib will use fast mode until a key is set.');
      }
    } else {
      ui.log.info('Run booklib init --reset later to configure API reasoning.');
    }
