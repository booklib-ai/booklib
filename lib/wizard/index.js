...
async function stepProcessingMode(ui, cwd) {
  ...
  if (hasApiKey) {
    options.push({ value: 'api', label: 'Cloud AI (recommended)', hint: `Uses your ${keyProvider} API key — best quality, ~$0.001/query` });
  } else {
    options.push({ value: 'api', label: 'Cloud AI (recommended)', hint: 'Uses API key (Anthropic or OpenAI) — best quality, ~$0.001/query' });
  }
