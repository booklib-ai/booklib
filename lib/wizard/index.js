...
import os from 'os';
import { confirm, multiSelect, progressBar, sep } from './prompt.js';
...
async function stepProjectDetection(cwd) {
  ...
  if (project.languages.length > 0) {
    ...
    if (!ok) {
      process.stdout.write('  Enter your tech stack (e.g. "Python FastAPI", "Kotlin Android"): ');
      const { createInterface } = await import('node:readline');
      const rl = createInterface({ input: process.stdin, output: process.stdout });
      const answer = await new Promise(r => rl.once('line', l => { rl.close(); r(l.trim()); }));
      return { languages: [answer], frameworks: [], signals: [] };
    }
  } else {
    process.stdout.write('  No language detected.\n');
    process.stdout.write('  What are you working on? (e.g. "Python backend", "Kotlin Android"): ');
    const { createInterface } = await import('node:readline');
    const rl = createInterface({ input: process.stdin, output: process.stdout });
    const answer = await new Promise(r => rl.once('line', l => { rl.close(); r(l.trim()); }));
    return { languages: [answer], frameworks: [], signals: [] };
...
async function stepSkillRecommendations(project) {
  ...
  const { createInterface } = await import('node:readline');
  const rl = createInterface({ input: process.stdin, output: process.stdout });
  const query = await new Promise(r => rl.once('line', l => { rl.close(); r(l.trim()); }));
