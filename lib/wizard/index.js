...
async function runSetup(cwd) {
  ...
  if (selectedAgents.length > 0) {
    ...
    for (const agent of selectedAgents) {
      ...
      if (agent === 'claude') {
        console.log(`    ${label} — automatic: injects context before edits,`);
        console.log('      catches contradictions after');
      } else if (['cursor', 'copilot', 'gemini', 'windsurf', 'roo-code', 'goose', 'opencode'].includes(agent)) {
        console.log(`    ${label} — MCP: AI calls BookLib when it needs context`);
      } else {
        console.log(`    ${label} — config file: guidelines written to project`);
      }
    }
    console.log('');
  }
