...
async function stepRecommendAndInstall(ui, project, slotsUsed, installedNames) {
  ...
  // List ALL available skills from bundled skills directory
  const { skillsPath } = resolveBookLibPaths();
  let allSkills = [];
  try {
    allSkills = fs.readdirSync(skillsPath)
      .filter(d => fs.existsSync(path.join(skillsPath, d, 'SKILL.md')))
      .filter(d => d !== 'skill-router'); // internal, not user-facing
  } catch {
    s.stop('Skills directory not found');
    return [];
  }

  s.stop(`${allSkills.length} skills available, ${recommendedNames.size} recommended for your stack`);
  ...
  const options = sortedSkills.map(name => {
    ...
    let hint = '';
    if (isRec && isInst) hint = 'recommended + installed';
    else if (isRec) hint = 'recommended';
    else if (isInst) hint = 'installed';
    else if (meta?.snippet) hint = meta.snippet;
    const score = meta ? ` [${meta.displayScore ?? Math.round(meta.score * 100)}%]` : '';
    return { value: name, label: `${name}${score}`, hint };
  });
  ...
  const selected = await ui.multiselect(
    `Skills for your project (${recommendedNames.size} pre-selected):`,
    options,
    { initialValues },
  );
