...
async function runSetup(cwd) {
  ...
  if (gaps?.postTraining?.length > 0) {
    if (gapResults.resolved > 0 && gapResults.unresolved === 0) {
      console.log(`  \u2713 ${gapResults.resolved} knowledge gap(s) resolved`);
    } else if (gapResults.resolved > 0) {
      console.log(`  \u2713 ${gapResults.resolved} knowledge gap(s) resolved`);
      console.log(`  \u26a0 ${gapResults.unresolved} gap(s) need manual attention (see suggestions above)`);
    } else {
      console.log(`  \u26a0 ${gaps.postTraining.length} post-training dep(s) — run: booklib resolve-gaps`);
    }
  }
  console.log('');
  console.log(`  ${sep()}`);
