...
async function runSetup(cwd) {
  ...
  try {
    ...
    // 2b. Profile selection — determines the section structure of generated config files
    const profile = await stepProfileSelection(session);
    ...
    // 3. Health check — diagnose slot problems (count ALL dirs, not just BookLib-managed)
    ...
    await stepWriteConfigs(session, cwd, selectedAgents, skillsForConfig, profile);
...
async function stepProfileSelection(session) {
  const profiles = [
    { value: 'software-development', label: 'Software development', hint: 'default' },
  ...
  profiles.forEach((p, i) => {
    ...
    process.stdout.write(`  ${i + 1}. ${p.label}${hint}\n`);
  });

  const picks = await session.numberedInput('\n  [1-5 or Enter for default]: ', profiles.length);
  const selected = picks.length > 0 ? profiles[picks[0]].value : 'software-development';
  process.stdout.write(`  Selected: ${profiles.find(p => p.value === selected).label}\n`);
  return selected;
}
...
async function stepRecommendAndInstall(session, project, slotsUsed, installedNames) {
  ...
  for (const r of results) {
    ...
    if (!bySkill.has(name) || r.score > bySkill.get(name).score) {
      ...
      bySkill.set(name, { name, score: r.score, displayScore: r.displayScore, description: r.metadata?.description ?? '', snippet });
    }
  }
  ...
  const choices = recommended.map(s => {
    ...
    const scoreDisplay = s.displayScore ?? Math.round((s.score ?? 0) * 100);
    return {
      label: s.name.padEnd(28) + `[${scoreDisplay}% match]${tag}`,
      description: s.snippet || s.description.slice(0, 60),
    };
  });
  ...
  // Add back the already-installed selections (they were skipped above)
  for (const name of selectedNames) {
    if (installedSet.has(name.toLowerCase()) && !installedSkillNames.includes(name)) {
      installedSkillNames.push(name);
    }
  }

  // Cleanup offer — if way over limit
  if (slotsUsed > SKILL_LIMIT && installedSkillNames.length > 0) {
    const toRemove = installedNames.filter(n => !installedSkillNames.includes(n));
    process.stdout.write(`\n  You have ${slotsUsed} skills but only need ~${installedSkillNames.length} for this project.\n\n`);
    ...
    if (choice === 'c') {
      ...
      process.stdout.write(`  ✓ Removed ${removed} skills. Kept ${installedSkillNames.length}.\n`);
    }
  }
...
async function stepWriteConfigs(session, cwd, selectedAgents, skillNames, profile) {
  ...
  try {
    const written = await initializer.init({ skills: skillNames, target, dryRun: false, quiet: true, profile });
    if (written.length > 0) {
