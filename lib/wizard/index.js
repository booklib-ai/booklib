...
async function runSetup(cwd) {
  ...
  const decisionCount = gapResults?.details?.length ?? 0;

  console.log(`  ${sep()}`);
  console.log('');
  console.log('  What happens now:');
  console.log('');
  console.log('  Just code as usual. BookLib watches in the background');
  console.log('  and speaks up only when it matters:');
  console.log('');
  if (postCount > 0) {
    console.log(`  \u2022 ${postCount} packages in your project were released after`);
    console.log(`    your AI's training data (${fileCount} files, ${apiCount} APIs affected)`);
    console.log('    BookLib corrects the AI before it writes outdated code');
    console.log('');
  }
  if (indexedSources.length > 0 || decisionCount > 0) {
    console.log('  \u2022 Your team\'s decisions, specs, and conventions are');
    console.log('    indexed — BookLib reminds the AI to follow them');
    console.log('');
    console.log('  \u2022 If the AI writes something that contradicts a team');
    console.log('    decision, BookLib flags it immediately');
    console.log('');
  }
  if (postCount === 0 && indexedSources.length === 0) {
    console.log('  \u2022 BookLib is monitoring your project. Connect team');
    console.log('    knowledge (Notion, GitHub, docs) to get more value.');
    console.log('');
  }
  console.log('  BookLib stays silent when everything is fine.');
  console.log('');

  // Explain integration level per tool
  const hasHookTool = selectedAgents.some(a => a === 'claude');
  const mcpTools = selectedAgents.filter(a => ['claude', 'cursor', 'copilot', 'gemini', 'windsurf', 'roo-code'].includes(a));
  const nonMcpTools = selectedAgents.filter(a => !mcpTools.includes(a));

  if (hasHookTool) {
    console.log('  How it works with your tools:');
    console.log('');
    console.log('  Claude Code      fully automatic — BookLib injects context');
    console.log('                   before each edit and catches contradictions');
    console.log('                   after (via PreToolUse/PostToolUse hooks)');
  }
  if (mcpTools.length > (hasHookTool ? 1 : 0)) {
    const otherMcp = mcpTools.filter(a => a !== 'claude').map(a => AGENT_LABELS[a]);
    if (otherMcp.length > 0) {
      console.log(`  ${otherMcp.join(', ').padEnd(19)}MCP tools available — your AI calls`);
      console.log('                   BookLib when it needs project context');
    }
  }
  if (nonMcpTools.length > 0) {
    const labels = nonMcpTools.map(a => AGENT_LABELS[a]);
    console.log(`  ${labels.join(', ').padEnd(19)}config file generated — guidelines`);
    console.log('                   written to project for the AI to follow');
  }
  console.log('');
  console.log('');
  console.log(`  ${sep()}`);
  ...
  console.log('');
  if (postCount > 0) {
    console.log('  booklib analyze              see which files and APIs your');
    console.log('                               AI may get wrong, per package');
  }
  console.log('  booklib gaps                 list outdated packages with');
  console.log('                               publish dates and versions');
  console.log('  booklib connect <source>     connect team knowledge:');
  console.log('                               Notion, GitHub, local docs');
  console.log('  booklib connect notion page <id>');
  console.log('  booklib connect github releases <owner/repo>');
  console.log('  booklib connect ./docs/');
  console.log('');
  console.log('  booklib search "query"       search indexed knowledge');
  console.log('  booklib check-imports <file>  check if imports are covered');
  console.log('  booklib doctor               check health and skill count');
  console.log('  booklib init --reset         reconfigure from scratch');
  console.log('');
...
async function stepBuildContextMap(ui, cwd, gaps) {
  ...
  try {
    ...
    // Collect knowledge items from graph nodes
