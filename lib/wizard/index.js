...
async function runSetup(cwd) {
  ...
  // Step 10: Auto-resolve knowledge gaps via Context7 / GitHub / manual
  ...
  // Step 11: Show project analysis — which APIs are affected by gaps
  if (gaps?.postTraining?.length > 0) {
  ...
  if (indexedSources.length > 0) {
    const totalFiles = indexedSources.reduce((sum, s) => sum + (s.files || 0), 0);
    const filePart = totalFiles > 0 ? `, ${totalFiles} files` : '';
    console.log(`  \u2713 ${indexedSources.length} doc source(s) indexed${filePart}`);
  }
  if (gaps?.postTraining?.length > 0) {
    if (gapResults.details?.length > 0) {
      for (const d of gapResults.details) {
        if (d.resolved) {
          console.log(`  \u2713 ${d.name}@${d.version} — ${d.pageCount} pages from ${d.source}`);
        } else {
          console.log(`  \u26a0 ${d.name}@${d.version} — needs manual connect`);
        }
      }
    }
    if (gapResults.unresolved > 0) {
      console.log(`  \u2192 Run: booklib resolve-gaps (or see suggestions above)`);
    }
  }
  console.log('');
  console.log(`  ${sep()}`);
  ...
  console.log('  booklib resolve-gaps           auto-resolve gaps');
  console.log('  booklib analyze                show affected APIs');
...
async function stepIndexBuild(ui) {
  ...
  // If a valid index already exists (vectra index + non-empty BM25), skip build
  ...
  // Vectra exists but BM25 is missing/empty — check backup
  if (vectraExists && !bm25Exists) {
...
async function stepConnectDocs(ui, cwd, uncapturedDocs) {
  ...
  for (const dir of [...pkmDirs, ...sddDirs]) {
    ...
    if (fs.existsSync(full) && fs.statSync(full).isDirectory()) {
      ...
      if (!docSources.some(d => d.path === sourcePath)) {
        const fileCount = fs.readdirSync(scanDir).filter(f => /\.(md|mdx|txt|yml|yaml)$/i.test(f)).length;
  ...
  for (const doc of selected) {
    ...
    try {
      ...
      const indexer = new BookLibIndexer();
      const result = await indexer.indexDirectory(sourcePath, false, { sourceName, quiet: true });

      mgr.markIndexed(sourceName, 0);
      s.stop(`${doc.path} \u2192 indexed as ${sourceType}`);
      indexed.push({ name: sourceName, files: result?.files ?? 0 });
    } catch (err) {
