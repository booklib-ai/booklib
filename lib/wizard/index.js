...
async function runSetup(cwd) {
  ...
  if (reasoningMode === 'api') {
    ui.log.info('Set ANTHROPIC_API_KEY or OPENAI_API_KEY in your environment for API reasoning.');
  }

  // Save reasoning mode to config
  try {
    const { configPath } = resolveBookLibPaths(cwd);
    let savedConfig = {};
    try { savedConfig = JSON.parse(fs.readFileSync(configPath, 'utf8')); } catch { /* no config */ }
...
async function stepProcessingMode(ui) {
  const mode = await ui.select('How should BookLib process search results?', [
    { value: 'fast', label: 'Fast (default)', hint: 'Filtered search results, no AI reasoning — works everywhere, instant' },
    { value: 'local', label: 'Local model', hint: 'Stricter relevance filtering — no extra cost, slightly better quality' },
    { value: 'api', label: 'API', hint: 'Uses your API key for AI reasoning — best quality, ~$0.001/query' },
  ]);
  return mode;
}
