...
export function createWizardUI() {
  return {
    ...
    async select(message, options) {
      const hint = '  (arrows to move, ENTER to confirm)';
      const result = await clack.select({ message: `${message}\n${hint}`, options });
      if (clack.isCancel(result)) { clack.outro('Setup cancelled.'); process.exit(0); }
      return result;
    },

    async multiselect(message, options, opts = {}) {
      const hint = '  (arrows to move, SPACE to select/deselect, ENTER when done)';
