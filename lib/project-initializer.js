...
export class ProjectInitializer {
   ...
   */
  async init({ skills, target = 'all', dryRun = false, quiet = false, onFileConflict, profile, stack } = {}) {
    ...
    for (const t of targets) {
      ...
      if (profile) {
        // Profile-based rendering — compact template with variable substitution
        const rendered = this._render(t, blocks, skillNames);
        filePath = rendered.filePath;
        fileHeader = rendered.fileHeader;
        const profileContent = this._renderFromProfile(t, profile, skillNames, stack);
        content = `${MARKER_START}\n${profileContent}\n${MARKER_END}`;
      } else {
        // Legacy rendering — full block extraction
   ...
   *
   * @param {string} _target - tool target (reserved for future per-tool customization)
   * @returns {string} markdown block with agent behavior instructions
   */
  _getAgentBehaviors(_target) {
    return `When reviewing code, query BookLib for relevant principles:\n  \`booklib search "topic"\`\n\nWhen the user wants to save an insight:\n  \`booklib capture --title "..." --type insight\``;
  }
  ...
  _getReferences(target) {
    ...
    lines.push('- [BookLib documentation](https://booklib-ai.github.io/skills/)');
    lines.push('- [BookLib skills catalog](https://github.com/booklib-ai/skills)');
    return '## References\n\n' + lines.join('\n');
  }
