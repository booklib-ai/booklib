...
export class ProjectInitializer {
  ...
  async init({ skills, target = 'all', dryRun = false } = {}) {
    ...
    for (const t of targets) {
      const { filePath, content } = this._render(t, blocks, skillNames);
      const absPath = path.join(this.projectCwd, filePath);
      if (dryRun) {
        console.log(`\n[dry-run] Would write: ${filePath}\n${'─'.repeat(60)}\n${content.slice(0, 400)}…`);
      } else {
        fs.mkdirSync(path.dirname(absPath), { recursive: true });
        // For CLAUDE.md: append a clearly delimited section rather than overwrite
        if (t === 'claude' && fs.existsSync(absPath)) {
          const existing = fs.readFileSync(absPath, 'utf8');
          const marker = '<!-- booklib-standards-start -->';
          if (existing.includes(marker)) {
            // Replace existing booklib section
            const replaced = existing.replace(
              /<!-- booklib-standards-start -->[\s\S]*?<!-- booklib-standards-end -->/,
              content
            );
