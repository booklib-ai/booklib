...
export class BookLibIndexer {
  ...
  async indexNodeFile(filePath, nodesDir) {
    ...
    if (chunks.length === 0) {
      const { data } = (await import('gray-matter')).default.read(filePath);
      ...
      await this.index.insertItem({
        vector,
        metadata: { text: fallbackText, title: data.title, type: data.type, nodeKind: 'knowledge', nodeFile: filePath }
      });
      return;
    }
