...
export class BookLibIndexer {
   ...
   * Adds nodeKind: 'knowledge' metadata so search can distinguish nodes from skills.
  ...
  async indexKnowledgeNodes(nodesDir) {
    ...
    for (const file of files) {
      const content = fs.readFileSync(file, 'utf8');
      const relativePath = path.relative(nodesDir, file);
      let chunks;
      try {
        chunks = parseSkillFile(content, relativePath);
      } catch (err) {
      ...
      for (const chunk of chunks) {
        ...
        await this.index.insertItem({
          vector,
          metadata: { ...chunk.metadata, text: chunk.text, nodeKind: 'knowledge', nodeFile: file }
        });
      }
    }
  }
