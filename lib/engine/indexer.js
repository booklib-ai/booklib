...
export class BookLibIndexer {
  ...
  async indexDirectory(dirPath, clearFirst = false, opts = {}) {
    const { quiet = false, onProgress, onFileProgress, sourceName } = opts;

    if (clearFirst && fs.existsSync(this.indexPath)) {
      fs.rmSync(this.indexPath, { recursive: true, force: true });
    }

    // Validate vectra index.json — if corrupt (0 bytes), delete and recreate.
    // Only check file size to avoid OOM on large index files.
    ...
    // Phase 3: Insert into vectra (one at a time — vectra has no batch insert)
    for (let i = 0; i < allChunks.length; i++) {
      const meta = { ...allChunks[i].metadata, text: allChunks[i].text };
      if (sourceName) meta.sourceName = sourceName;
      await this.index.insertItem({ vector: vectors[i], metadata: meta });
    }

    const bm25 = clearFirst ? new BM25Index() : this._loadOrCreateBM25();
