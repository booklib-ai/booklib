...
export class BM25Index {
  ...
  _appendDoc(chunk) {
    ...
    this._docs.push({ text: chunk.text, metadata: chunk.metadata, freq: freqObj, len: tokens.length });

    const totalLen = this._docs.reduce((sum, doc) => sum + doc.len, 0);
    this._avgLen = totalLen / this._docs.length;
  }
}
