// Suppress ONNX runtime warnings BEFORE the native module loads
// CoreML logs partial-support warnings to stderr — harmless but noisy
 ...
 * Priority: DirectML (Windows) > CUDA (Linux) > CPU.
 *
 * CoreML on macOS is intentionally skipped — it produces uncontrollable native
 * warnings ("Context leak", "IsInputSupported") that spam stderr. The MiniLM
 * model is small enough that CPU with int8 quantization is fast and silent.
 *
...
export async function detectProvider() {
  ...
  try {
    ...
    if (typeof ort.listSupportedBackends === 'function') {
      ...
      // Skip CoreML — produces uncontrollable native stderr warnings
...
export async function createEmbeddingPipeline(opts = {}) {
  ...
  if (providerInfo.provider !== 'cpu') {
    ...
    if (!quiet) {
      console.log(`  Acceleration: ${providerInfo.name}`);
    }
  }
  ...
  if (providerInfo.provider === 'cpu') {
    ...
    if (!quiet) {
      const tip = providerInfo.suggestion
        ? `\n  Tip: ${providerInfo.suggestion.split('\n')[0]}`
        : '';
      console.log(`  Acceleration: CPU (quantized int8 — 2x faster than default)${tip}`);
    }
  }
