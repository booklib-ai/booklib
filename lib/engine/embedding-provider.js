...
export async function createEmbeddingPipeline(opts = {}) {
  ...
  if (isMac) {
    // Redirect native stderr during model load to suppress CoreML warnings
    ...
    try {
      ...
      if (!quiet) console.log('  Using CoreML (Apple Silicon)');
    } catch {
      // CoreML failed — fall back to CPU
      extractor = await pipeline('feature-extraction', MODEL_NAME, { dtype: 'q8' });
      if (!quiet) console.log('  Using CPU (int8 quantized)');
    } finally {
      process.stderr.write = origStderrWrite; // restore
    }
  } else {
    extractor = await pipeline('feature-extraction', MODEL_NAME, { dtype: 'q8' });
  }
