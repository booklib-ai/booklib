...
export async function createEmbeddingPipeline(opts = {}) {
  ...
  if (isMac) {
    // CoreML spams native warnings ("Context leak", "IsInputSupported") via NSLog.
    // Suppress at two levels: OS_ACTIVITY_MODE for NSLog, console.warn for ONNX runtime.
    const origWarn = console.warn;
    const origActivity = process.env.OS_ACTIVITY_MODE;
    ...
    try {
      ...
      if (origActivity === undefined) delete process.env.OS_ACTIVITY_MODE;
      else process.env.OS_ACTIVITY_MODE = origActivity;
    }
  } else {
    extractor = await pipeline('feature-extraction', MODEL_NAME, { dtype: 'q8' });
  }
