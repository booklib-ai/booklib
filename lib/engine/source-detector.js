...
export function detectSourceType(dirPath, opts = {}) {
  ...
  for (const [type, rules] of Object.entries(HEURISTICS)) {
    ...
    scores[type] = score + (dirBonus[type] ?? 0);
  }
...
function detectDirSignals(dirPath) {
  const bonus = {};
  const DIR_SIGNALS = {
    '.obsidian': { type: 'pkm', score: 20 },
    '.logseq': { type: 'pkm', score: 20 },
    '.foam': { type: 'pkm', score: 20 },
    '.specify': { type: 'sdd-spec', score: 20 },
    '.planning': { type: 'sdd-spec', score: 20 },
    '.gsd': { type: 'sdd-spec', score: 20 },
    '.kiro': { type: 'sdd-spec', score: 20 },
  };
  try {
