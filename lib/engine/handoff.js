...
export class BookLibHandoff {
  ...
  saveState({ name, goal, next, progress, skills }) {
    ...
    try {
      ...
      // Team decisions and post-training gaps from context map
      const mapPath = path.join(booklibDir, 'context-map.json');
      if (fs.existsSync(mapPath)) {
        const map = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
        if (map.items) {
          const decisions = map.items.filter(i => i.type === 'decision');
          const gaps = map.items.filter(i => i.type === 'post-training');
          decisions.forEach(d => knowledgeItems.push(`<decision>${d.id}</decision>`));
          gaps.forEach(g => knowledgeItems.push(`<post_training_gap>${g.id}</post_training_gap>`));
        }
      }
