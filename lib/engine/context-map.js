...
export class ContextMapBuilder {
  ...
  async buildFromGaps(gaps) {
    ...
    const mapped = gaps.map(gap => ({
      ...
      functionPatterns: [],
      injection: {
        correction: `${gap.name}@${gap.version ?? 'latest'} (published ${gap.publishDate ?? 'unknown'}). Post-training.`,
        constraint: null,
        example: null,
      },
    }));
    ...
    try {
      ...
      for (const entry of parsed) {
        ...
        if (Array.isArray(entry.functionPatterns)) {
          target.functionPatterns = entry.functionPatterns;
        }
        if (Array.isArray(entry.importTriggers)) {
          const merged = new Set([...target.importTriggers, ...entry.importTriggers]);
          target.importTriggers = [...merged];
        }
      }
    } catch {
...
export class ContextMapMatcher {
  ...
  match(filePath, codeBlock, imports) {
    ...
    for (const item of this.items) {
      ...
      if (strength > 0) scored.push({ ...item, _strength: strength });
    }
  ...
  _scoreItem(item, filePath, codeLower, importSet) {
    ...
    const importMatch = this._matchImports(item.importTriggers, importSet);
    if (importMatch) strength += STRENGTH_IMPORT;

    const fnMatch = this._matchFunctionPatterns(item.functionPatterns, codeLower);
    if (fnMatch) strength += STRENGTH_FUNCTION;

    const termMatch = this._matchCodeTerms(item.codeTerms, codeLower);
    if (termMatch) strength += STRENGTH_CODE_TERM;

    const fileMatch = this._matchFilePatterns(item.filePatterns, filePath);
    if (fileMatch) strength += STRENGTH_FILE_PATTERN;

    // Post-training items with only '**' glob must have importTriggers match
    if (item.type === 'post-training' && this._hasOnlyWildcard(item.filePatterns) && !importMatch) {
      return 0;
    }
  ...
  _matchImports(triggers, importSet) {
    ...
    return triggers.some(t => importSet.has(t.toLowerCase()));
  }
